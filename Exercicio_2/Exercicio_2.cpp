...
void Terra(void){
	if(terra) //controle da cor de terra
...
void Casa(void){		
	...
	if(casa) //controle da cor da parede da casa
...
void Arvore(void){
	...
	if(arvore) //controle da cor da arvore
		glColor3f(0.6, 0.3, 0.0);  // Marrom
	else
		glColor3f(1.0,0.0,0.0);  // Vermelho
...
void Desenha(void){
	if(fundo) //controle da cor de fundo
...
void Teclado (unsigned char key, int x, int y) {
	if(key == 27)
		exit(0);
	else if(key == 70 or key == 102) // F
		fundo = not(fundo);
	else if(key == 84 or key == 116) // T
		terra = not(terra);
	else if(key == 67 or key == 99)	// C
		casa = not(casa);
	else if(key == 65 or key == 97)	// A
		arvore = not(arvore);
	else if(key == 83 or key == 115) // S
		sol = not(sol);
	else if(key == 79 or key == 111){ // O
		fundo = true;
		terra = true;
		casa = true;
		arvore = true;
